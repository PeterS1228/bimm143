...
ggplot(cdc) +
  ...
  geom_point() +
  geom_line() +
  geom_vline(xintercept = 1942, col = "blue", linetype = 2) + 
  geom_vline(xintercept = 1980, col = "gray", linetype = 2) + 
  geom_vline(xintercept = 1992, col = "red", linetype = 2) 
```
...
```{r}
ab <- read_json("http://cmi-pb.org/api/ab_titer", simplifyVector = T)
head(ab)
```
...
There are way less visit 8 specimens because the project is still ongoing and we have not gotten that data for all individuals yet.

## Examin IgG1 Ab titer levels
...
Clearly FIM2/3 changes. This is "Fimbrial protein" that makes the bacteria plius and is involved in cell adhesion.

PT Pertussis toxin

FHA is Filamentous hemagglutinin

etc.

adding infancy_vac to the faceting
